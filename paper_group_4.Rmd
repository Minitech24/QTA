---
title: "QTA Paper Group 4"
author: "Daniil Bulat, Kevin Hardegger, Christoph Heinimann"
date: "03/06/2020"
output:
  pdf_document:
    number_sections: yes
...
# read data 
data <- read_csv(here("scraped_nzz_csv","nzz_2020_2013.csv"))

# change Date columns to date type
data = data %>%
  mutate(Date_Link = as.Date(data$Date_Link, "%d. %m. %Y")) %>%
  mutate(Date = as.Date(data$Date, "%d. %m. %Y"))

# remove duplicates
data = data[!duplicated(data$Title),]

# use Date_Link value where Date values are missing
data$Date[is.na(data$Date)] <- data$Date_Link[is.na(data$Date)]
...
data$Teaser <- recode(data$Teaser, "False"="", .default = data$Teaser)

# mutate Text column combining Title and Teaser & add monthYear 
data = data %>%
  mutate(Text = paste(Title, Teaser, sep = " ")) %>%
  mutate(monthYear = format(Date, "%Y-%m")) %>%
  mutate(Year = format(Date, "%Y"))

# remove Title, Teaser and Date_Link columns
data = data[,-(2:4)] # You might encounter the error "Unknown or uninitialised column: `Date_Link`" from here on out. This appears to be an error with the diagnostics tool in RStudio, not our code specifically (see https://stackoverflow.com/questions/39041115/fixing-a-multiple-warning-unknown-column).

# create random sample for labeling
# set.seed(1234)
# 
# rows <- sample(nrow(data))
# 
# write.csv(data[rows, ],
#           file="labeled_set.csv",
#           fileEncoding = "utf-8",
#           row.names = FALSE)

# combine with labeled data set
labeled = read_csv("labeled_set.csv")

# join labeled columns to main data set
data = left_join(data, labeled, by = "ID")

# delete & remove labeled for tidiness reasons
rm(labeled)

```

```{r ML setup}
# remove NA values to create training set
dataTrain = data %>%
  mutate(Text = as.character(data$Text)) %>%
  filter(!is.na(immi_label)) # filtering for one of the two labels is enough, as the same rows are labeled

# creating corpus   
train_corpus = corpus(dataTrain$Text)
...
# Abstract
In recent years, the Neue Zürcher Zeitung (NZZ), a major newspaper based in Switzerland, has come under fire from allegations that its content and political view have shifted increasingly to the political right. This paper examines these claims by analyzing headlines and teasers of NZZ articles based on their sentiment and evaluates if such a shift truly came into effect. For this, a total of 17'095 articles of the front pages of nzz.ch between 2013 and 2020 are scraped from the web. To test the claims made, the data is filtered into two topics, immigration and the far-right German party "Alternative für Deutschland" (AfD), which is then submitted to detailed exploratory analysis. The results show that neither a specific trend nor pattern can be found that would justify the accusations. On the contrary, the findings suggests that, if anything, a shift in the opposite direction seems to be occurring.
...
The last chart depicts the influence AfD had on the overall sentiment of NZZ. There is one big spike visible that reaches `r max(SentInflu_afd$Influence)`%, which is likely related to the case of Hans-Georg Maaßen and his involvement with AfD. However, in general, the influence is rather low, ranging between 0% and 3% with a mean of `r round(mean(SentInflu_afd$Influence),2)`%. Its important to point out that the 52 months with no related articles aren't contained in this chart. For these months, influence would of course be 0%.

# Conclusion
From the above, it becomes clear that a number of our secondary hypotheses can easily be falsified. Addressing the first two, the opposite even appears to be the case, with reporting on immigration becoming more positive and reporting on AfD more negative in terms of the number of articles written. Both of these secondary hypotheses can therefore clearly be rejected. Addressing the last two, the influence of immigration does not seem to increase substantially over time, with only intermittent spikes appearing where important events occur that would probably receive similar attention by other publications. The influence of AfD topics, at the same time, does seem to increase, which is in line with our fourth secondary hypothesis. It must be considered, however, that AfD as a party only came into under the spotlight in the wake of the refugee crisis in 2015-2016 and saw its first political breakthroughs in the parliamentary elections of 2017. Therefore, its influence on NZZ reporting increasing recently is not of such extraordinary significance.

All in all, the data shows our primary hypothesis can soundly be rejected. Measured by the metrics we defined, no shift to the political right can be discerned from the sentiment expressed in the data analysed. By the standards of this analysis, if anything, NZZ seems to slowly report more positively on immigration and more negatively on the AfD. One possible explanation for the allegations raised is that, although they were raised against NZZ, they might be more related to the persons involved in its publication than the actual output of the newspaper itself.

# Caveats
Our present analysis only considers titles and, as available, teasers of articles, not the actual content of articles themselves. This has a number of drawbacks:

1. Though NZZ is not known as a regular user of click-bait, titles and teasers often follow a particular style that might not be reflected in the articles they advertise. As their purpose is capturing the reader's attention, they might resort to more drastic language that exaggerates the positions held in the article itself. At the same time, however, as accusations against NZZ also contend that the newspaper looks to cater more to certain audiences, the way NZZ styles these headlines and teasers might be telling of an at least projected political stance all the same, even if it is less emphasized in the content of the articles itself.
2. Besides that, titles and teasers obviously provide significantly less data than the full text of an article would. Accordingly, this paper only analyses a small snippet of the message NZZ communicates on their website. Any conclusive analysis would have to take into account the full breadth, or at least significantly more, of NZZ's journalistic output.
3. A lot of our results (like spikes in graphs) are hard to explain, owing in our view mostly to a lack of relevant data. This means that a lot of results don't necessarily express specific real-world happenings, with a lot of spikes being nothing more than noise, drastically limiting our possibilities for interpretation. This appears to be both a shortcoming of the scraper, which quite simply can not capture all of nzz.ch, as well as our machine learning algorithm to identify relevant articles, which we know has missed a number of instances. More sophisticated approaches to both could most likely yield results that can be interpreted with greater confidence.

# References
Aschenbrenner, C., Assmann, A., Assmann, J., Bartels, K., Benhabib, S., Bitterli, U., Böhme, G., & Böhme, H. (2017). *Protestbrief NZZ Feuilleton* [Open letter]. https://www.persoenlich.com/sites/default/files/filemanager/Protestbrief%20NZZ%20Feuilleton.pdf

Binswanger, D. (2019, July 13). Die neuen Freunde der NZZ. *Republik.ch*. https://www.republik.ch/2019/07/13/die-neuen-freunde-der-nzz

Brandt, H. (11.07.2019). Die NZZ umwirbt deutsche Rechtspopulisten. *Tagesanzeiger*. https://www.tagesanzeiger.ch/kultur/die-lieblingszeitung-der-rechten/story/17731189

Daum, M., & Shaller, C. (16.12.2017). ‘Neue Zürcher Zeitung’: Druck von rechts. *Die Zeit*. https://www.zeit.de/2017/52/neue-zuercher-zeitung-christoph-blocher-rechtsruck

Maaßen, H.-G. (2019, July 9). Hans-Georg Maaßen on Twitter: ‘Für mich ist die NZZ so etwas wie „Westfernsehen“. (hgm) https://t.co/XAom2oKVbR’. *Twitter*. https://twitter.com/HGMaassen/status/1148654208398319622

Mäurer, D. K. (2019, July 17). ‘Neue Zürcher Zeitung’—Profil geschärft—Mit rechten Thesen? *Deutschlandfunk.* https://www.deutschlandfunk.de/neue-zuercher-zeitung-profil-geschaerft-mit-rechten-thesen.2907.de.html?dram:article_id=454110

Niggemeier, S. (2019, July 10). Mit dem ‘Westfernsehen’ sieht man schlechter. *Übermedien*. https://uebermedien.de/39948/mit-dem-westfernsehen-sieht-man-schlechter/

Obrist, H. (2019, July 10). Warum dieser NZZ-Artikel für einen Shitstorm sorgte – und er von Maassen retweetet wurde. *St.Galler Tagblatt*. https://www.tagblatt.ch/international/warum-dieser-nzz-artikel-fuer-einen-shitstorm-sorgte-und-er-von-maassen-retweetet-wurde-ld.1134396

Rasch, M. (2019a, July 9). In deutschen Städten sieht die Mehrheitsgesellschaft ihrem Ende entgegen. *Neue Zürcher Zeitung*. https://www.nzz.ch/international/in-deutschen-staedten-geht-die-mehrheitsgesellschaft-zu-ende-ld.1492568

Rasch, M. (2019b, July 9). Michael Rasch on Twitter: ‘Frankfurt ist erst der Anfang: In deutschen Städten sieht die #Mehrheitsgesellschaft ihrem Ende entgegen https://t.co/1BsI1bTY2n via @nzz #urdeutsche #biodeutsche #Migration #Migrationshintergrund #Ausländer’. *Twitter*. https://twitter.com/raschnzz/status/1148509043180343296

Rauchfleisch, A., Vogler, D., & Eisenegger, M. (2017). *Auf zu neuen Märkten: Der Einfluss Schweizer Medien auf Twitter.* https://www.foeg.uzh.ch/de/analyse/alleanalysen/Auf-zu-neuen-M%C3%A4rkten-Der-Einfluss-Schweizer-Medien-auf-Twitter.html

Rauh, C. (2018). Validating a sentiment dictionary for German political language—A workbench note. *Journal of Information Technology & Politics*, 15(4), 319–343. https://doi.org/10/gdsgns

Schmidt, C. (2018, February 28). NZZ: Warum das Blatt sich wendet. *NDR.de*. https://www.ndr.de/fernsehen/sendungen/zapp/NZZ-Warum-Blatt-sich-wendet,nzz126.html 

Surber, K. (12.10.2017). NZZ: Die Angst geht um an der Falkenstrasse. *Die Wochenzeitung*. https://www.woz.ch/-81a3

von Matt, R. (2019, July 18). Deutschland-Strategie der NZZ - «Von einem Rechtsrutsch ist nichts zu spüren». *Schweizer Radio und Fernsehen (SRF)*. https://www.srf.ch/news/schweiz/deutschland-strategie-der-nzz-von-einem-rechtsrutsch-ist-nichts-zu-spueren
